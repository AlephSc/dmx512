...
### Input fisik (hardware button deck) — NONAKTIF default (v50)

> **v50**: deck dikompilasi keluar lewat `HW_DECK_ENABLE 0` di `dmx_web_rgb.ino`.
> Alasan: pengujian A/B terkontrol membuktikan task input fisik menjadi
> penyebab "lagging/strobo lambat" di rig nyata (build tanpa-HW terbukti
> normal; detail: `docs/logs.md`). Kode v49.x lengkap tetap ada di repo —
> set `HW_DECK_ENABLE 1` untuk mengaktifkan kembali. Saat nonaktif: pin tak
> dikonfigurasi, tak ada task tambahan, serial `HWOFF`/`HWON` menjawab
> `hw_disabled`, Web UI menampilkan "DECK FISIK: NONAKTIF".

Spesifikasi bila diaktifkan (`HW_DECK_ENABLE 1`):
...
- **5 lapis anti-noise EMI** (dikembangkan dari kasus nyata): common-mode detector (≥2 pin serempak = noise), debounce dua-arah 60 ms, rate-limit, pin-lockout + lockout global dengan log pin bermasalah, kill switch `HWOFF/HWON`.
- **Trigger-split latensi**: B2/B3 = go-button (play saat ditekan), B1/B4 = dual-fungsi (play saat dilepas, hold = pindah grup).
...
- Dual-core dipisah tugas: **DMX engine Core 0** (prio 18 — bebas kelaparan oleh traffic jaringan), Web/serial Core 1. *(Task dedikasi input fisik prio 12 hanya ada bila deck diaktifkan.)*
...
**Deck tombol fisik (opsional, NONAKTIF default sejak v50):**
...
(jauhkan dari kabel daya/dimmer, dan jangan sejalur kabel tombol dgn kabel DMX/daya)

Aktifkan: set HW_DECK_ENABLE 1 di dmx_web_rgb.ino, lalu compile ulang.
(Default 0: pin tak disentuh firmware — wiring boleh tetap terpasang, aman.)
```
...
6. **Deck fisik**: NONAKTIF di build v50 (penyebab lagging di rig); aktifkan via `HW_DECK_ENABLE 1` bila deck terpasang.
...
- [x] Deck tombol fisik + encoder + 5 lapis anti-noise EMI (v49) — dinonaktifkan default di v50 (penyebab lagging terkonfirmasi A/B; kode tetap ada)
